...
| **investing-like-pro** | 投資/交易類工具。含 `gooaye`(以股癌視角分析/挑股,內建逐字稿萃取的「標的記憶」,每週自動更新)與 `ctbc-securities-api`(用 Python+pywin32 操作中國信託證券交易 API:登入/下單/查詢/回報,含 headless client 與回傳解析腳本)。**教育用途,非投資建議;交易涉及真錢,務必先用測試環境。** |
